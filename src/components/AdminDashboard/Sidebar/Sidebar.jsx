  ...
  return (
    <div className='bg-white rounded-2xl  shadow-2xl h-screen  '>
      <div className='flex flex-col items-center justify-center '>
        ...
        <button onClick={handleDashboard} className={`${isOpen?"":"bg-red-900 bg-opacity-40"} rounded-full p-2`}><IoMenu className='text-2xl'  /></button>
          ...
          <ul className='pt-6'>
            {AdminSideBarData.map(item => (
              <li key={item.id} className="flex flex-col font-normal">
                ...
                >
                  <div className='flex  '>
                    {item.icon && <item.icon className="inline-block  w-6 h-6" />}
                  </div>
                  <div className={`ml-4 mr-2 ${isOpen?"":"hidden"}`}> {item.name}</div>
                </Link>
              </li>
            ))}
          </ul>
