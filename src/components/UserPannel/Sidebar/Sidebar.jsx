...
const Sidebar = ({ isOpen, handleDashboard }) => {
  ...
  return (
    <div className="bg-zinc-900 font-abc   shadow-inner h-screen  flex flex-col justify-between ">
        ...
        <span className="text-red-600 font-semibold pt-2"> collab</span>
          ...
          <ul className="pt-8">
            {SideBarData.map((item) => {
              if (item.userType && user?.userType !== item.userType)
                return null;
              return (
      ...
      <div className="">
      <div className={`flex items-center  p-2 bg-zinc-950 ${isOpen?' justify-between':'justify-center'}`}>
        
        <div className={`${isOpen?'flex flex-col item-center justify-center ':'hidden'}`}>
