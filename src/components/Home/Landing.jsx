...
const Landing = () => {
  ...
  return (
    ...
    <div className={`pl-8 flex flex-col items-center justify-center transition-all duration-200 ease-in ${  isHovered ? 'bg-black' : 'bg-white'}`}>
    <div className={`text-6xl  font-bold  ${  isHovered ? 'scale-50 transition-all duration-200 ease-out ' : ''}`}>Crisis Collab</div>
    <div className={`text-xl text-semibold text-gray-400 ${  isHovered ? 'scale-90 transition-all duration-200 ease-out ' : ''}`}> Connecting Rescue Relief Agencies with the world</div>
    ...
    <img src={mid} alt="Your Image" className={` m-24  max-w-full rounded-full  ${  isHovered ? 'h-[299px] w-[299px] scale-90 transition-all duration-200 ease-in' : 'h-[170px] w-[170px]'} `} />
      ...
      <div className={`flex-none  h-full absolute right-0 w-1/2 transition-all duration-200 ease-in  ${  isHovered ? 'bg-black' : 'bg-white'}` }>
        <div className={`bg-black h-full w-full   rounded-tl-[1000px] transition-all duration-200 ease-in ${  isHovered ? 'bg-white' : 'bg-black'}`} onMouseEnter={handleHover}
